//As  with  any program, execution has to start at the CPU. We will create test data, process it on the CPU (host), 
//time it and transfer the data to the GPU (device), process it again and copy the results back to the host machine.
